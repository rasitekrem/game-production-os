...
- Render refusals, never silent repairs: unknown scene, no authored camera, frame out of range, an unavailable or non-built-in engine (detected from Blender's own load report), an oversized resolution, any external dependency, Freestyle (which runs scripts even with auto-execution off), OSL script nodes, compositor File Output nodes, multi-view, sequencer strips and stamp burn-in. PNG metadata stamps are switched off in memory, so renders are reproducible and name no file.
