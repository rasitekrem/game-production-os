...
MUTATIONS = [
    ...
    ('all failures collapse into one status', 'gpos/tools/diagnostics.py',
     ...
     'EXIT_FOR = {SUCCESS: 0, INVALID_REQUEST: 1, FAILED: 1, TIMED_OUT: 1, UNAVAILABLE: 1,\n            CONFLICT: 1, INCOMPATIBLE: 1, CANCELLED: 1, INTERNAL_ERROR: 1, OUTCOME_UNKNOWN: 1}'),
    ...
    # --- Phase 2C-6A: SESSION leases, lease modes, OUTCOME_UNKNOWN
    ('a SESSION lease is judged stale from the attaching command pid', 'gpos/tools/leases.py',
     '    if scope_of(record) != EXECUTION:  # the command that took a SESSION lease has long exited\n        return False\n', ''),
    ('a batch writer meeting a session gets the old conflict and stale report', 'gpos/tools/execution.py',
     '        if exc.code == "LEASE_CONFLICT" and lease_mod.scope_of(exc.holder) == lease_mod.SESSION:\n            # A live session',
     '        if False:\n            # A live session'),
    ('session verification ignores the owner', 'gpos/tools/leases.py',
     '    if owner_id is not None and record.get("owner_id") != owner_id:', '    if False:'),
    ('session verification ignores the session id', 'gpos/tools/leases.py',
     '    if bound.get("session_id") != session_id:', '    if False:'),
    ('the session owner drops the actor kind', 'gpos/tools/leases.py',
     '    owner = f"{actor.kind}:{actor.id}"', '    owner = f"AGENT:{actor.id}"'),
    ('an opened but unconfirmed session outlives its execution', 'gpos/tools/execution.py',
     '        if self.opened is None or self.confirmed:\n            return []', '        if True:\n            return []'),
    ('a session id is accepted by a capability without sessions', 'gpos/tools/execution.py',
     '        if not capability.takes_session_id:\n            out.append(f"{cap} names no existing session, so a session_id has no meaning for it")',
     '        if False:\n            out.append(f"{cap} names no existing session, so a session_id has no meaning for it")'),
    ('a session capability runs without a session id', 'gpos/tools/execution.py',
     '    elif capability.takes_session_id:\n        out.append(f"{cap} acts on an existing session and needs session_id")', ''),
    ('a session capability runs without an owner', 'gpos/tools/execution.py',
     '    if capability.session_mode and lease_mod.session_owner(request.actor) is None:', '    if False:'),
    ('a lease break ignores the inspected token', 'gpos/tools/leases.py',
     '    if expected_token is not None and (record is None or record.get("token") != expected_token):', '    if False:'),
    ('SESSION_REQUIRED takes a writer lease (modes conflated)', 'gpos/tools/execution.py',
     '    if mode == "EXECUTION" and not request.dry_run:', '    if capability.single_writer_required and not request.dry_run:'),
    ('a capability reaches a lease operation of another mode', 'gpos/tools/execution.py',
     '        if self.capability.effective_lease_mode not in modes:\n            raise AssertionError', '        if False:\n            raise AssertionError'),
    ('a READ_ONLY capability may claim a writer lease to require a session', 'gpos/tools/validation.py',
     '    if mode == "SESSION_REQUIRED" and cap.mutating != cap.single_writer_required:', '    if False:'),
    ('a STATELESS capability may use a session', 'gpos/tools/validation.py',
     '    if cap.state_model != "STATEFUL":\n        _problem(out, adapter_id, f"{cap.id} uses a session, so it must be STATEFUL", cap.id)', ''),
    ('OUTCOME_UNKNOWN shares the FAILED exit code', 'gpos/tools/diagnostics.py',
