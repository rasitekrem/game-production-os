...
- A path counts as Blender's own bundled asset only when the file it resolves to lies inside the running installation's resolved datafiles directory; stored path text is never trusted. PNG metadata stamps are switched off in memory, so renders are reproducible and name no file.
