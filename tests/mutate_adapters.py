...
MUTATIONS = [
    ...
    ('instruction layers ignored on sync', 'gpos/adapters/pipeline.py', '    diags += _layer_conflicts(root, backend, set(owned_entry_files) | {backend.entrypoint})', '    pass'),
    ('instruction layers ignored on check', 'gpos/adapters/pipeline.py', '    out = _layer_conflicts(root, backend, owned_entrypoints(root, BACKENDS.values(), _manifest_or_none))', '    out = []'),
